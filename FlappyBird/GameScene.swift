...
class GameScene: SKScene, SKPhysicsContactDelegate{
    ...
    //効果音
    let action = SKAction.playSoundFileNamed("decision-5.mp3", waitForCompletion: true)
    ...
    let itemDefaults:UserDefaults = UserDefaults.standard
    
    //SKView状にシーンが出されたときに呼び出されすメソッド
    ...
    func setupWall(){
        ...
        let createWallAnimation = SKAction.run({
            ...
            wall.zPosition = -50.0 //雲より手前地面より奥
            
            //画面のy軸の中央値
            let center_y = self.frame.size.height / 2
            //壁のy座標を上下ランダムにさせる時の最大値
            let randome_y_range = self.frame.size.height / 4
            //下の壁のy軸の下限
            let under_wall_lowest_y = UInt32( center_y - wallTexture.size().height / 2 - randome_y_range / 2)
            //1~randome_yまでのランダムな整数を生成
            let randome_y = arc4random_uniform( UInt32(randome_y_range))
            //yの値にランダムな値を足して、下の壁のy座標を決定
            let under_wall_y = CGFloat(under_wall_lowest_y + randome_y)
    ...
    //SKContactDelegateのメソッド。衝突した時に呼ばれる
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory{
            ...
            //アイテムベストスコア更新か確認をする
            var bestItemScore = itemDefaults.integer(forKey: "itemBest")
            if itemScore > bestItemScore{
                bestItemScore = itemScore
                itemBestScoreLabelNode.text = "ItemBest:\(bestItemScore)"
                itemDefaults.set(bestItemScore, forKey: "itemBest")
                itemDefaults.synchronize()
            }
        }
        else{
           //壁か地面と衝突した
    ...
    func restart(){
        ...
        bird.position = CGPoint(x: self.frame.size.width * 0.2, y: self.frame.size.width * 0.7)
        bird.physicsBody?.velocity = CGVector.zero
        bird.physicsBody?.collisionBitMask = groundCategory | wallCategory
        bird.zRotation = 0.0
    ...
    func setupApple(){
        ...
        //リンゴを生成するアクションを作成
        let createAppleAction = SKAction.run({
            let apple = SKNode()
            apple.position = CGPoint(x:self.frame.size.width + appleTexture.size().width / 2.0, y: 0.0)
            apple.zPosition = -51
            ...
            let randome_y_range = self.frame.size.height / 4
            //リンゴの下限
            let under_item_lowest_y = UInt32( center_y - appleTexture.size().height / 2 - randome_y_range / 2)
            //ランダムな値を設定
            let randome_y = arc4random_uniform(UInt32(randome_y_range))
            let under_item_y = CGFloat(under_item_lowest_y + randome_y)
            //リンゴを作成
            let appleItem = SKSpriteNode(texture: appleTexture)
            appleItem.position = CGPoint(x:0.0, y: under_item_y)
            //物理演算を設定
            appleItem.physicsBody = SKPhysicsBody(circleOfRadius: appleItem.size.height)
            appleItem.physicsBody?.categoryBitMask = self.itemScoreCategory
            appleItem.physicsBody?.contactTestBitMask = self.birdCategory
        ...
        //次のリンゴ作成までの待ち時間のアクションを作成
        let waitAnimation = SKAction.wait(forDuration: 2)
        
        //無限に切り替える
        let repeatForeverAnimation = SKAction.repeatForever(SKAction.sequence([createAppleAction, waitAnimation]))
    ...
    func setupItemScoreLabel(){
        ...
        let bestItemScore = itemDefaults.integer(forKey: "itemBest")
        itemBestScoreLabelNode.text = "Item Best:\(bestItemScore)"
